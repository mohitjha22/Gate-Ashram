...
export class TopictestComponent implements OnInit {
  ...
  constructor(
    private _topictestService: TopictestService,
    private router: Router,
    private route: ActivatedRoute
  ) { }

  ngOnInit() {
    this.data = JSON.parse(this.route.snapshot.paramMap.get('topics'));

    this._topictestService.getTestQuestions(this.data).
      subscribe((data) => {
        this.questions = data;
        //console.log(data);
  ...
  submitTest() {
    ...
    var correct2 = 0;
    var score = 0;
    var heading = "Topicswise Test-" + this.data.subject;
